...
- **タイトルバーに開いたファイル名を表示**: ウィンドウタイトルにファイル名を動的に表示、複数ファイルの同時編集時に便利
- **高度なフィルタリング機能**:
  - 全列または特定列に対する検索
  - 大文字小文字を区別しない部分一致検索
...
- **ファイル再読み込み**: 🔄ボタンで開いているファイルを最新の状態に更新
...
1. **ファイルを開く**: 
   ...
   - ウィンドウタイトルに開いたファイル名が表示されます（例: `TSV Log Viewer - sample_log.tsv`）

2. **ファイルを閉じる**:
   - メニューバーから `File → Close` を選択するか (Ctrl+W)
   - テーブルがクリアされ、新しいファイルを開く準備ができます
   - ウィンドウタイトルがデフォルトの `TSV Log Viewer` にリセットされます

3. **ファイルを再読み込み**:
   - ツールバーの 🔄 再読み込みボタンをクリック
   - 現在開いているファイルを最新の状態で再度読み込みます
   - ログファイルが別のプロセスで更新された場合に便利です
   - フィルタ条件はそのまま保持され、新しいデータに再適用されます
   - ボタンはファイルを開いている時のみ有効になります
...
5. **ソート**: 列ヘッダをクリックして昇順・降順を切り替え

6. **詳細表示**: 行をダブルクリックで全フィールドの詳細を表示

7. **コピー**: Ctrl+Cで選択したセルの内容をコピー

8. **列の管理**: 右クリックまたはメニューから列の表示/非表示を設定

9.  **データエクスポート**: フィルタされたデータを File → Export で出力

10. **検索機能**:
    - 検索欄に文字を入力して検索実行（Enter キーまたは「検索」ボタンで確定）
    - 一致した行番号が黄色でハイライト
    - 「前へ」「次へ」ボタンで一致箇所を移動
    - 「Clear」ボタンで検索をリセット
    - 検索中に文字を編集しても、確定までは前回のハイライトを保持して効率的
...
| **Ctrl+O** | ファイルを開く |
| **Ctrl+W** | ファイルを閉じる |
| **Ctrl+C** | セルの内容をコピー（Ctrl + セルクリック） |
| **Ctrl+Shift+S** | 別名でファイルをエクスポート |
| **Ctrl+F** | 検索欄にフォーカス（文字列があれば全選択） |
| **Ctrl+G** | 指定行へジャンプ |
| **Enter** | 検索を実行 |
| **F3** | 次の検索結果へ移動 |
| **Shift+F3** | 前の検索結果へ移動 |
...
- **エンコーディング**: UTF-8
- **区切り文字**: タブ (`\t`)
- **拡張子**: すべてのファイル形式に対応（テキストファイルのみ）
- バイナリファイルを選択した場合は、エラーが表示されます
- 各行が1レコードとして扱われます
